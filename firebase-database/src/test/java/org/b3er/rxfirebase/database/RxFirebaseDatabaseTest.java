...
public class RxFirebaseDatabaseTest {
  ...
  @Mock DataSnapshot mockDataSnapshot;

  @Mock DatabaseError mockDatabaseError;

  @Mock MutableData mockMutableData;

  @Mock Function<MutableData, Transaction.Result> mockTransactionTask;
  ...
  @Test public void testChildEvents_add() {
    ...
    for (Object event : events) {
      ...
      assertThat(event).isInstanceOf(ChildAddEvent.class);
      assertThat(((ChildAddEvent) event).previousChildName()).isEqualTo("foo");
    }
  ...
  @Test public void testChildEvents_move() {
    TestObserver<ChildEvent> sub = TestObserver.create();
    ...
    for (Object event : events) {
      assertThat(event).isInstanceOf(ChildEvent.class);
      assertThat(event).isInstanceOf(ChildMoveEvent.class);
      assertThat(((ChildMoveEvent) event).previousChildName()).isEqualTo("foo");
    }
  ...
  @Test public void testData_onCancelled() {
    TestObserver<DataSnapshot> sub = TestObserver.create();
  ...
  @Test public void testDataChanges() {
    TestObserver<DataSnapshot> sub = TestObserver.create();
  ...
  @Test public void testRemoveValue() {
    ...
    RxFirebaseDatabase.removeValue(mockDatabaseReference).subscribe(sub);
  ...
  @Test public void testRemoveValue_Unsuccessful() {
    when(mockDatabaseReference.removeValue()).thenReturn(mockTask);
  ...
  @Test public void testSetPriority() {
    when(mockDatabaseReference.setPriority(1)).thenReturn(mockTask);

    TestObserver sub = TestObserver.create();

    RxFirebaseDatabase.setPriority(mockDatabaseReference, 1).subscribe(sub);
  ...
  @Test public void testSetPriority_notSuccessful() {
    when(mockDatabaseReference.setPriority(1)).thenReturn(mockTask);
  ...
  @Test public void testSetValue() {
    when(mockDatabaseReference.setValue(1)).thenReturn(mockTask);

    TestObserver sub = TestObserver.create();

    RxFirebaseDatabase.setValue(mockDatabaseReference, 1).subscribe(sub);
  ...
  @Test public void testSetValue_notSuccessful() {
    when(mockDatabaseReference.setValue(1)).thenReturn(mockTask);
  ...
  @Test public void testSetValueWithPriority_notSuccessful() {
    when(mockDatabaseReference.setValue(1, 1)).thenReturn(mockTask);
  ...
  @Test public void testRunTransaction() throws Exception {
    TestObserver sub = TestObserver.create();

    RxFirebaseDatabase.runTransaction(mockDatabaseReference, mockTransactionTask).subscribe(sub);
  ...
  @Test public void testRunTransaction_onError() {
    TestObserver sub = TestObserver.create();
  ...
  private void verifyAddChildEventListener() {
    verify(mockDatabaseReference).addChildEventListener(childEventListener.capture());
  }

  private void verifyAddListenerForSingleValueEvent() {
    verify(mockDatabaseReference).addListenerForSingleValueEvent(valueEventListener.capture());
  }

  private void verifyAddValueEventListener() {
    verify(mockDatabaseReference).addValueEventListener(valueEventListener.capture());
  }

  private void verifyRunTransaction() {
    verify(mockDatabaseReference).runTransaction(transactionHandler.capture(), anyBoolean());
  }

  private void verifyTransactionTaskCall() throws Exception {
    verify(mockTransactionTask).apply(mockMutableData);
  }

  private void verifyAddOnCompleteListenerForTask() {
    //noinspection unchecked
    verify(mockTask).addOnCompleteListener(onCompleteListener.capture());
  }
  ...
  private <T> void callValueEventOnDataChange(T value) {
    when(mockDataSnapshot.getValue(value.getClass())).thenReturn(value);
  ...
  private void callValueEventOnCancelled(DatabaseException mockedException) {
    when(mockDatabaseError.toException()).thenReturn(mockedException);
  ...
  private void callTaskOnCompleteWithError(Exception e) {
    ...
    when(mockTask.getException()).thenReturn(e);
