...
public class RxFirebaseUserTest {
  ...
  @Mock UserProfileChangeRequest mockProfileChangeRequest;

  @Mock Task<Void> mockVoidTaskResult;

  @Mock Task<GetTokenResult> mockGetTokenTaskResult;

  @Mock Task<AuthResult> mockAuthTaskResult;
  ...
  @Test public void testLinkWithCredential() {
    mockSuccessfulAuthResult();
    when(mockFirebaseUser.linkWithCredential(mockAuthCredential)).thenReturn(mockAuthTaskResult);

    TestObserver<AuthResult> obs = TestObserver.create();

    RxFirebaseUser.linkWithCredential(mockFirebaseUser, mockAuthCredential).subscribe(obs);
  ...
  @Test public void testLinkWithCredential_notSuccessful() {
  ...
  @Test public void testReload_notSuccessful() {
    mockNotSuccessfulVoidResult(new IllegalStateException());
    when(mockFirebaseUser.reload()).thenReturn(mockVoidTaskResult);
  ...
  @Test public void testSendEmailVerification() {
    mockVoidResult(true);
    when(mockFirebaseUser.sendEmailVerification()).thenReturn(mockVoidTaskResult);

    TestObserver obs = TestObserver.create();

    RxFirebaseUser.sendEmailVerification(mockFirebaseUser).subscribe(obs);
  ...
  @Test public void testSendEmailVerification_notSuccessful() {
    mockNotSuccessfulVoidResult(new IllegalStateException());
    when(mockFirebaseUser.sendEmailVerification()).thenReturn(mockVoidTaskResult);
  ...
  @Test public void testUpdateEmail() {
    mockVoidResult(true);
    when(mockFirebaseUser.updateEmail("foo@bar.com")).thenReturn(mockVoidTaskResult);

    TestObserver obs = TestObserver.create();

    RxFirebaseUser.updateEmail(mockFirebaseUser, "foo@bar.com").subscribe(obs);
  ...
  @Test public void testUpdateProfile_notSuccessful() {
    mockNotSuccessfulVoidResult(new IllegalStateException());
    when(mockFirebaseUser.updateProfile(mockProfileChangeRequest)).thenReturn(mockVoidTaskResult);
  ...
  private void mockVoidResult(boolean success) {
    when(mockVoidTaskResult.isSuccessful()).thenReturn(success);

    //noinspection unchecked
    when(mockVoidTaskResult.addOnCompleteListener(onComplete.capture())).thenReturn(
        mockVoidTaskResult);
  }
  ...
  private void mockSuccessfulTokenResult(String token) {
    ...
    when(mockGetTokenTaskResult.isSuccessful()).thenReturn(true);

    when(mockGetTokenTaskResult.getResult()).thenReturn(mockGetTokenResult);

    //noinspection unchecked
    when(mockGetTokenTaskResult.addOnCompleteListener(onComplete.capture())).thenReturn(
        mockGetTokenTaskResult);
  }

  private void mockNotSuccessfulTokenResult(Exception exception) {
    when(mockGetTokenTaskResult.isSuccessful()).thenReturn(false);

    when(mockGetTokenTaskResult.getException()).thenReturn(exception);
  ...
  private void mockSuccessfulAuthResult() {
    when(mockAuthTaskResult.isSuccessful()).thenReturn(true);

    when(mockAuthTaskResult.getResult()).thenReturn(mockAuthResult);

    //noinspection unchecked
    when(mockAuthTaskResult.addOnCompleteListener(onComplete.capture())).thenReturn(
        mockAuthTaskResult);
  }

  private void mockNotSuccessfulAuthResult(Exception exception) {
    when(mockAuthTaskResult.isSuccessful()).thenReturn(false);

    when(mockAuthTaskResult.getException()).thenReturn(exception);
  ...
  private void mockNotSuccessfulVoidResult(Exception exception) {
    when(mockVoidTaskResult.isSuccessful()).thenReturn(false);

    when(mockVoidTaskResult.getException()).thenReturn(exception);
  }
